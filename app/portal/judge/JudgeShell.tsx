...
function isActive(pathname: string, href: string): boolean {
  ...
  if (h === "/portal/judge") {
    return p === "/portal/judge";
  }
  return p === h;
}

const NAV: { href: string; label: string }[] = [
  { href: "/portal/judge/", label: "Dashboard" },
    ...
    <div className="admin-shell">
      ...
      <div className="admin-shell__rail">
        <header className="admin-shell__mobile-topbar">
          <Link href="/portal/judge/" className="admin-shell__mobile-brand" onClick={closeNav}>
            <Image src={LOGO_SRC} alt="" width={120} height={36} className="admin-shell__logo-img" unoptimized />
            <span className="admin-shell__mobile-brand-text">Judge</span>
          </Link>
          <button
            type="button"
            className="admin-shell__menu-trigger"
            aria-expanded={navOpen}
            aria-controls="judge-sidebar-nav"
        ...
        <aside className={`admin-shell__sidebar ${navOpen ? "is-open" : ""}`} id="judge-sidebar">
          <div className="admin-shell__brand">
            <Link href="/portal/judge/" className="admin-shell__logo-link" onClick={closeNav}>
              <Image
                src={LOGO_SRC}
          ...
          <nav className="admin-shell__nav" id="judge-sidebar-nav" aria-label="Judge navigation">
