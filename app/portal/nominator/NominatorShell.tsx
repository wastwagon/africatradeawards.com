...
function isActive(pathname: string, href: string): boolean {
  ...
  if (h === "/portal/nominator") {
    return p === "/portal/nominator";
  }
  return p === h;
}

const NAV: { href: string; label: string }[] = [
  { href: "/portal/nominator/", label: "Dashboard" },
  { href: "/portal/entrant/", label: "Entrant workspace" },
  { href: "/vote/", label: "Public voting" },
  { href: "/", label: "Website home" },
];

export default function NominatorShell({
  email,
  fullName,
  children,
}: {
    ...
    <div className="admin-shell">
      ...
      <div className="admin-shell__rail">
        <header className="admin-shell__mobile-topbar">
          <Link href="/portal/nominator/" className="admin-shell__mobile-brand" onClick={closeNav}>
            <Image src={LOGO_SRC} alt="" width={120} height={36} className="admin-shell__logo-img" unoptimized />
            <span className="admin-shell__mobile-brand-text">Nominator</span>
          </Link>
          <button
            type="button"
            className="admin-shell__menu-trigger"
            aria-expanded={navOpen}
            aria-controls="nominator-sidebar-nav"
        ...
        <aside className={`admin-shell__sidebar ${navOpen ? "is-open" : ""}`} id="nominator-sidebar">
          <div className="admin-shell__brand">
            <Link href="/portal/nominator/" className="admin-shell__logo-link" onClick={closeNav}>
              <Image
                src={LOGO_SRC}
          ...
          <nav className="admin-shell__nav" id="nominator-sidebar-nav" aria-label="Nominator navigation">
