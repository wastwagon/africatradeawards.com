...
const NAV: { href: string; label: string }[] = [
  { href: "/portal/voter/", label: "My dashboard" },
  { href: "/vote/", label: "Public voting" },
  { href: "/", label: "Website home" },
];

export default function VoterShell({
  email,
  fullName,
  children,
}: {
    ...
    <div className="admin-shell">
      ...
      <div className="admin-shell__rail">
        <header className="admin-shell__mobile-topbar">
          <Link href="/portal/voter/" className="admin-shell__mobile-brand" onClick={closeNav}>
            <Image src={LOGO_SRC} alt="" width={120} height={36} className="admin-shell__logo-img" unoptimized />
            <span className="admin-shell__mobile-brand-text">Voter</span>
          </Link>
          <button
            type="button"
            className="admin-shell__menu-trigger"
            aria-expanded={navOpen}
            aria-controls="voter-sidebar-nav"
        ...
        <aside className={`admin-shell__sidebar ${navOpen ? "is-open" : ""}`} id="voter-sidebar">
          <div className="admin-shell__brand">
            <Link href="/portal/voter/" className="admin-shell__logo-link" onClick={closeNav}>
              <Image
                src={LOGO_SRC}
          ...
          <nav className="admin-shell__nav" id="voter-sidebar-nav" aria-label="Voter navigation">
          ...
          <div className="admin-shell__logout-wrap">
            <LogoutButton className="admin-shell__logout" redirectTo="/vote/">
              Log out
            </LogoutButton>
          </div>
        </aside>
